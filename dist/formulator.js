...
/***/ function(module, exports, __webpack_require__) {
	...
	var Formulator = function () {
		...
		_createClass(Formulator, [{
			...
			key: "getTpl",
			value: function getTpl(name) {
				if (!this.defaultTheme) return null;
				var theme = this.themes[this.defaultTheme];

				// Check for theme.
				if (name.indexOf(".") != -1) {
					var n = name.substr(0, name.indexOf("."));
					if (n in this.themes) {
						theme = this.themes[n];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _formulator = __webpack_require__(1);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Element = function (_React$Component) {
	  ...
	  function Element(props) {
	    ...
	    if (!('autoValidate' in props) && 'form' in _this.context) {
	      _this.props.autoValidate = _this.context.props.autoValidate;
	    }

	    // Determine fieldset render function.
	    _this.fieldsetRenderFunc = null;
	    if (_this.props.fieldset === true) {
	      _this.fieldsetRenderFunc = Formulator.getTpl("Fieldset");
	      if (!_this.fieldsetRenderFunc) {
	        throw new Error("Formulator: current theme does not have a Fieldset() template");
	      }
	    } else if (_typeof(_this.props.fieldset) === "object") {
	      _this.fieldsetRenderFunc = _this.props.fieldset;
	    }

	    // Determine render function.
	    var tpl = props.tpl || _this.constructor.name;
	    _this.renderFunc = Formulator.getTpl(props.tpl) || _this._render;
	    if (!_this.renderFunc) {
	      throw new Error("Formulator: Could not determine render function for '" + _this.constructor.name + "' - no _render() func");
	    }
